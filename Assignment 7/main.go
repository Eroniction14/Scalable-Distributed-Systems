...
// paymentSem limits concurrency to 1 at a time, each taking 3s.
// This is the "creative" approach from the assignment: a buffered
// channel of size 1 acts as a semaphore so even though goroutines
// don't truly block on time.Sleep, only one can hold the token.
var paymentSem = make(chan struct{}, 1)

func verifyPayment(orderID string) {
	paymentSem <- struct{}{}        // acquire
	defer func() { <-paymentSem }() // release
