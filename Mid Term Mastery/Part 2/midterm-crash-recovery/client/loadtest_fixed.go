...
func main() {
	// Now hitting the PROXY on port 9090 instead of server directly
	serverURL := "http://localhost:9090/albums"
	totalRequests := 70
	results := []Result{}

	fmt.Println("=== Load Test: Albums Service (WITH Circuit Breaker) ===")
	fmt.Printf("Target: %s (via Circuit Breaker Proxy)\n", serverURL)
	...
	file, _ := os.Create("results_with_fix.json")
	defer file.Close()
